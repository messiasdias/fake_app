<template>
    <ul class="col s12 pagination" >
            ...
            <li v-if="(meta.currentPage - 1) >= 1"  class="waves-effect">
                <a title="First" @click.prevent="$store.dispatch('all',1)" >
                <i class="material-icons">chevron_left</i></a>
            </li>
            
            <li  v-if="(meta.currentPage - 1) >= 1" class="waves-effect" >
                <a @click.prevent="$store.dispatch('all', (meta.currentPage - 1) )">
                    {{meta.currentPage - 1 }}
                </a>
            </li>

             <li class="waves-effect active" >
                <a @click.prevent="$store.dispatch('all', meta.currentPage )">
                    {{meta.currentPage }}
                </a>
            </li>
